...
namespace _3.PresentationLayers.Views
{
    public partial class Form_BanHang : Form
    {
            ...
            if (hd.TrangThai == 0)
            {
                if (!string.IsNullOrEmpty(tb_vidiem.Text))
                {
                    ...
                    else if (!string.IsNullOrEmpty(tb_sodienthoai.Text))
                    {
                        ...
                        if (validatePhoneNumberRegex.IsMatch(tb_sodienthoai.Text) == false)
                        {
        ...
        private void cbb_nganhang_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (cbb_nganhang.SelectedIndex > 0)
            {
                kh1 = _ikhachHangService.getAll().FirstOrDefault(x => x.Ma == cbb_nganhang.SelectedItem);
                tb_diachi.Text = kh1.DiaChi;
                tb_sodienthoai.Text = kh1.Sdt;
                tb_nguoinhan.Text = kh1.Ten;
            }
            else
            {
                tb_diachi.Clear();
